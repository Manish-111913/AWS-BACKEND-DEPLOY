...
class DataSeed {
  ...
  async createMenu() {
    ...
    // Ensure "Snacks" category exists
    await this.pool.query(
      `INSERT INTO menucategories (name, business_id, is_active) VALUES ('Snacks', $1, true) ON CONFLICT (business_id, name) DO NOTHING`,
      [this.businessId]
    );

    // Get the IDs for "Appetizers" and "Snacks"
    const appetizersCat = await this.pool.query(`SELECT category_id FROM menucategories WHERE name = 'Appetizers' AND business_id = $1`, [this.businessId]);
    const snacksCat = await this.pool.query(`SELECT category_id FROM menucategories WHERE name = 'Snacks' AND business_id = $1`, [this.businessId]);

    if (appetizersCat.rows.length > 0 && snacksCat.rows.length > 0) {
      const appetizersId = appetizersCat.rows[0].category_id;
      const snacksId = snacksCat.rows[0].category_id;

      // Update menu items to point to "Snacks"
      await this.pool.query(
        `UPDATE menuitems SET category_id = $1 WHERE category_id = $2`,
        [snacksId, appetizersId]
      );

      // Delete the "Appetizers" category
      await this.pool.query(
        `DELETE FROM menucategories WHERE category_id = $1`,
        [appetizersId]
      );

      console.log('✅ "Appetizers" category migrated to "Snacks".');
    }
